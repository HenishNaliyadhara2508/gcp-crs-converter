(function (global, factory) {
    typeof exports === 'object' && typeof module !== 'undefined'
        ? (module.exports = factory(require('wkt-parser')))
        : typeof define === 'function' && define.amd
          ? define(['wkt-parser'], factory)
    ...
    */
    ...
    function UTMtoLL(utm) {
        ...
        N1 =
            a /
            Math.sqrt(1 - eccSquared * Math.sin(phi1Rad) * Math.sin(phi1Rad));
    ...
    function init$3() {
        ...
        np = Math.pow(n, 2);
        this.Qn =
            (this.k0 / (1 + n)) * (1 + np * (1 / 4 + np * (1 / 64 + np / 256)));
    ...
    function forward$8(p) {
        ...
        if (Math.abs(Math.abs(lat) - HALF_PI) <= EPSLN) {
            ...
            var ul =
                (sl * Math.sin(this.gamma0) - vl * Math.cos(this.gamma0)) / tl;
        ...
        if (this.no_rot) {
            ...
            us -= this.uc;
            p.x =
                this.x0 + vs * Math.cos(this.alpha) + us * Math.sin(this.alpha);
            p.y =
                this.y0 + us * Math.cos(this.alpha) - vs * Math.sin(this.alpha);
        }
        return p;
    }
        ...
        if (this.sphere) {
            ...
            if (Math.abs(this.sin_p12 - 1) <= EPSLN) {
                ...
                lon = adjust_lon(
                    this.long0 +
                        Math.asin(
                            (Math.sin(Az) * Math.sin(Ee)) / Math.cos(psi),
                        ),
                );
                sinpsi = Math.sin(psi);
                lat = Math.atan2(
