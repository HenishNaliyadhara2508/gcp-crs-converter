    ...
    parse: function (data) {
        ...
        for (var i = 0; i < lines.length; i++) {
            ...
            if (cmd === 'G0' || cmd === 'G1') {
                var line = {
                    x:
                        args.x !== undefined
                            ? absolute(state.x, args.x)
                            : state.x,
                    y:
                        args.y !== undefined
                            ? absolute(state.y, args.y)
                            : state.y,
                    z:
                        args.z !== undefined
                            ? absolute(state.z, args.z)
                            : state.z,
                    e:
                        args.e !== undefined
                            ? absolute(state.e, args.e)
                            : state.e,
                    f:
                        args.f !== undefined
                            ? absolute(state.f, args.f)
                            : state.f,
                };
        ...
        function addObject(vertex, extruding) {
            ...
            geometry.setAttribute(
                'position',
                new Float32BufferAttribute(vertex, 3),
            );
