...
WorkerExecutionSupport.prototype = {
    ...
    setTerminateWorkerOnLoad: function (terminateWorkerOnLoad) {
        ...
        if (
            ...
            if (this.logging.enabled) {
                console.info(
                    'Worker is terminated immediately as it is not running!',
                );
            }
    ...
    executeParallel: function (payload, transferables) {
        ...
        payload.defaultGeometryType =
            this.worker.workerRunner.defaultGeometryType;
    ...
    _verifyWorkerIsAvailable: function (payload, transferables) {
        ...
        if (this.worker.queuedMessage !== null) {
            console.warn(
                'Already processing message. Rejecting new run instruction',
            );
            ready = false;
        } else {
    ...
    _postMessage: function () {
        if (this.worker.queuedMessage !== null) {
            if (
                this.worker.queuedMessage.payload.data.input instanceof
                ArrayBuffer
            ) {
                let transferables = [];
                if (this.worker.forceWorkerDataCopy) {
                    ...
                    transferables.push(
                        this.worker.queuedMessage.payload.data.input,
                    );
                }
