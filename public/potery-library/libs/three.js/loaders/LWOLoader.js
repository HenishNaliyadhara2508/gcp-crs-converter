...
MaterialParser.prototype = {
    ...
    parseConnections(connections, nodes) {
        ...
        inputName.forEach(function (name, index) {
            if (name === 'Material') {
                var matNode = scope.getNodeByRefName(
                    inputNodeName[index],
                    nodes,
                );
        ...
        nodeName.forEach(function (name, index) {
            if (name === materialConnections.name) {
                materialConnections.maps[inputName[index]] =
                    scope.getNodeByRefName(inputNodeName[index], nodes);
            }
        });
    ...
    parseAttributeImageMaps(attributes, textures, maps) {
        for (var name in attributes) {
            ...
            if (attribute.maps) {
                ...
                var path = this.getTexturePathByIndex(
                    mapData.imageIndex,
                    textures,
                );
                if (!path) return;
    ...
    parsePhongAttributes(params, attributes, maps) {
            ...
            if (attributes['Color Highlight']) {
                params.specular = new Color()
                    ...
                    .lerp(
                        params.color
                            .clone()
                            .multiplyScalar(attributes.Specular.value),
                        attributes['Color Highlight'].value,
                    );
            } else {
                params.specular = new Color().setScalar(
        ...
        if (params.specular && attributes.Glossiness)
            params.shininess =
                7 + Math.pow(2, attributes.Glossiness.value * 12 + 2);
    },
