    ...
    MMDLoader.prototype = Object.assign(Object.create(Loader.prototype), {
        ...
        load: function (url, onLoad, onProgress, onError) {
            ...
            this[modelExtension === 'pmd' ? 'loadPMD' : 'loadPMX'](
                ...
                function (data) {
                    onLoad(
                        builder.build(data, resourcePath, onProgress, onError),
                    );
                },
                onProgress,
                onError,
        ...
        loadVPD: function (url, isUnicode, onLoad, onProgress, onError) {
            ...
            this.loader
                .setMimeType(
                    isUnicode ? undefined : 'text/plain; charset=shift_jis',
                )
                .setPath(this.animationPath)
        ...
        build: function (data) {
            ...
            for (var i = 0; i < data.metadata.boneCount; i++) {
                ...
                var bone = {
                    ...
                    scl: [1, 1, 1],
                    rigidBodyType:
                        boneTypeTable[i] !== undefined ? boneTypeTable[i] : -1,
                };
                ...
                for (var i = 0; i < data.metadata.boneCount; i++) {
                    ...
                    for (var j = 0, jl = ik.links.length; j < jl; j++) {
                        ...
                        if (ik.links[j].angleLimitation === 1) {
                            ...
                            link.rotationMin = new Vector3().fromArray(
                                rotationMin,
                            );
                            link.rotationMax = new Vector3().fromArray(
                                rotationMax,
                            );
                        }
            ...
            function updateAttributes(attribute, morph, ratio) {
                for (var i = 0; i < morph.elementCount; i++) {
                    ...
                    attribute.array[index * 3 + 0] +=
                        element.position[0] * ratio;
                    attribute.array[index * 3 + 1] +=
                        element.position[1] * ratio;
                    attribute.array[index * 3 + 2] +=
                        element.position[2] * ratio;
                }
            }
            ...
            for (var i = 0; i < data.metadata.constraintCount; i++) {
                ...
                if (bodyA.type !== 0 && bodyB.type === 2) {
                    if (
                        bodyA.boneIndex !== -1 &&
                        bodyB.boneIndex !== -1 &&
                        data.bones[bodyB.boneIndex].parentIndex ===
                            bodyA.boneIndex
                    ) {
                        bodyB.type = 1;
                    }
    ...
    MaterialBuilder.prototype = {
        ...
        build: function (data, geometry /*, onProgress, onError */) {
            ...
            for (var i = 0; i < data.metadata.materialCount; i++) {
                ...
                params.morphTargets =
                    geometry.morphTargets.length > 0 ? true : false;
                params.fog = true;
                ...
                if (
                    data.metadata.format === 'pmx' &&
                    (material.flag & 0x1) === 1
                ) {
                    params.side = DoubleSide;
                } else {
                    params.side =
                        params.opacity === 1.0 ? FrontSide : DoubleSide;
                }
                ...
                if (data.metadata.format === 'pmd') {
                    ...
                    params.gradientMap = this._loadTexture(
                        ...
                        textures,
                        {
                            ...
                            isDefaultToonTexture:
                                this._isDefaultToonTexture(toonFileName),
                        },
                    );
                    ...
                    if (material.toonIndex === -1 || material.toonFlag !== 0) {
                        toonFileName =
                            'toon' +
                            ('0' + (material.toonIndex + 1)).slice(-2) +
                            '.bmp';
                        isDefaultToon = true;
                    } else {
    ...
    AnimationBuilder.prototype = {
        ...
        buildSkeletalAnimation: function (vmd, mesh) {
            ...
            for (var key in motions) {
                ...
                var basePosition = mesh.skeleton
                    .getBoneByName(key)
                    .position.toArray();
        ...
        _createTrack: function (
            ...
            if (times.length > 2) {
                ...
                for (
                    ...
                    for (var i = 0; i < stride; i++) {
                        if (
                            values[index * stride + i] !==
                                values[(index - 1) * stride + i] ||
                            values[index * stride + i] !==
                                values[aheadIndex * stride + i]
                        ) {
                            index++;
                            break;
                        }
                    ...
                    if (aheadIndex > index) {
                        ...
                        for (var i = 0; i < interpolateStride; i++) {
                            interpolations[index * interpolateStride + i] =
                                interpolations[
                                    aheadIndex * interpolateStride + i
                                ];
                        }
                    }
                }
            ...
            track.createInterpolant =
                function InterpolantFactoryMethodCubicBezier(result) {
        ...
        Object.create(Interpolant.prototype),
        {
            ...
            interpolate_: function (i1, t0, t, t1) {
                ...
                if (stride === 4) {
                    ...
                    result[0] =
                        values[offset0] * (1 - ratio) + values[offset1] * ratio;
                }
