...
class KTX2Container {
    constructor(basisModule, arrayBuffer) {
        ...
        var tablesByteOffset =
            selectorsByteOffset + this.sgd.selectorsByteLength;
    ...
    async initMipmaps(transcoder, config) {
        ...
        if (config.astcSupported) {
            ...
            texFormat === TextureFormat.UASTC4x4
        ) {
        ...
        for (var level = 0; level < this.header.levelCount; level++) {
            ...
            for (
                ...
                if (result.transcodedImage === undefined) {
                    throw new Error(
                        'THREE.KTX2Loader: Unable to transcode image.',
                    );
                }
                ...
                var levelData = result.transcodedImage
    ...
    getAlpha() {
        ...
        if (this.getTexFormat() === TextureFormat.UASTC4x4) {
            ...
            if (
                (this.dfd.samples[0].channelID & 0xf) ===
                DFDChannel.UASTC.RGBA
            ) {
                return true;
            }
