...
(function (global, factory) {
    ...
    var slice_slice = function (length) {
        ...
        if (this._index + length <= this._array.length) {
            return Promise.resolve(
                this._array.subarray(this._index, (this._index += length)),
            );
        }
    ...
    function slice(source) {
        return typeof source.slice === 'function'
            ? source
            : new SliceSource(
            ...
            return value && value[0] !== 0x1a
                ? {
                      ...
                      value: that._fields.reduce(function (p, f) {
                          p[f.name] = types[f.type](
                              that._decode(value.subarray(i, (i += f.length))),
                          );
                          return p;
                      }, {}),
                  }
    ...
    var dbf = function (source, decoder) {
        ...
        return source.slice(32).then(function (array) {
            var head = view(array);
            return source
                .slice(head.getUint16(8, true) - 32)
                .then(function (array) {
    ...
    function Dbf(source, decoder, head, body) {
        ...
        for (var n = 0; body.getUint8(n) !== 0x0d; n += 32) {
            ...
            this._fields.push({
                name: this._decode(
                    new Uint8Array(body.buffer, body.byteOffset + n, j),
                ),
    ...
    var shp_read = function () {
        ...
        return that._source.slice(8).then(function (array) {
            ...
            return that._source
                .slice(header.getInt32(4, false) * 2)
                .then(function (array) {
                    var record = view(array);
                    return {
                        done: false,
                        value: record.getInt32(0, true)
                            ? that._type(record)
                            : readNull(),
                    };
                });
    ...
    function open(shp$$1, dbf$$1, options) {
        if (typeof dbf$$1 === 'string') {
            ...
            dbf$$1 instanceof ArrayBuffer ||
            dbf$$1 instanceof Uint8Array
        ) {
            dbf$$1 = array(dbf$$1);
        } else if (dbf$$1 != null) {
            dbf$$1 = stream(dbf$$1);
        ...
        if (typeof shp$$1 === 'string') {
            ...
            if (dbf$$1 === undefined)
                dbf$$1 = path(
                    shp$$1.substring(0, shp$$1.length - 4) + '.dbf',
                    options,
                ).catch(function () {});
            shp$$1 = path(shp$$1, options);
        } else if (
            shp$$1 instanceof ArrayBuffer ||
            shp$$1 instanceof Uint8Array
        ) {
            shp$$1 = array(shp$$1);
        } else {
            shp$$1 = stream(shp$$1);
        ...
        return Promise.all([shp$$1, dbf$$1]).then(function (sources) {
            ...
            return shapefile(
                shp$$1,
                dbf$$1,
                dbf$$1 && new TextDecoder(encoding),
            );
        });
    }
