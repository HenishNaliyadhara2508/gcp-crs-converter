...
function readUsingDataView(event) {
    ...
    for (let i = 0; i < numPoints; i++) {
        ...
        let returnNumberAndNumberOfReturns = sourceView.getUint8(
            i * sourcePointSize + 14,
            true,
        );
        ...
        rangeNumberOfReturns[0] = Math.min(
            rangeNumberOfReturns[0],
            numberOfReturn,
        );
        rangeNumberOfReturns[1] = Math.max(
            rangeNumberOfReturns[1],
            numberOfReturn,
        );

        // CLASSIFICATION
        let classification = sourceView.getUint8(
            i * sourcePointSize + 15,
            true,
        );
        classifications[i] = classification;
        rangeClassification[0] = Math.min(
            rangeClassification[0],
            classification,
        );
        rangeClassification[1] = Math.max(
            rangeClassification[1],
            classification,
        );

        // POINT SOURCE ID
        let pointSourceID = sourceView.getUint16(
            i * sourcePointSize + 18,
            true,
        );
        pointSourceIDs[i] = pointSourceID;
