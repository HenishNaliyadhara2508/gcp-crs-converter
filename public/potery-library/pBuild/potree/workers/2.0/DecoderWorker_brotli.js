...
function BrotliDecodeClosure() {
    ...
    function unpackCommandLookupTable(cmdLookup) {
        ...
        for (var /** number */ cmdCode = 0; cmdCode < 704; ++cmdCode) {
            ...
            cmdLookup[index + 0] =
                INSERT_LENGTH_N_BITS[insertCode] |
    ...
    function initState(s, input) {
        ...
        s.blockTrees[0] = 7;
        s.distRbIdx = 3;
        var /** number */ maxDistanceAlphabetLimit =
    ...
    function maybeReallocateRingBuffer(s) {
        ...
        var /** !Int8Array */ newBuffer = new Int8Array(
    ...
    function readMetablockHuffmanCodesAndContextMaps(s) {
        ...
        s.literalBlockLength = readMetablockPartition(
            s,
            0,
            s.numLiteralBlockTypes,
        );
        ...
        s.literalTreeGroup = decodeHuffmanTreeGroup(
            256,
            256,
            numLiteralTrees,
            s,
        ...
        var /** number */ distanceAlphabetSizeMax =
        ...
        while (s.runningState != 10) {
            switch (s.runningState) {
                ...
                case 4:
                    ...
                    var /** number */ insertAndCopyExtraBits =
                            CMD_LOOKUP[cmdCode];
                    var /** number */ insertLengthOffset =
                            CMD_LOOKUP[cmdCode + 1];
                    var /** number */ copyLengthOffset =
                            CMD_LOOKUP[cmdCode + 2];
                ...
                case 7:
                    if (s.trivialLiteralContext != 0) {
                        ...
                        while (s.j < s.insertLength) {
                            ...
                            var /** number */ literalTreeIdx =
                                    s.contextMap[
                                        s.contextMapSlice + literalContext
                                    ] & 0xff;
                            s.literalBlockLength--;
                            prevByte2 = prevByte1;
                            if (s.bitOffset >= 16) {
                    ...
                    if (distanceCode < 0) {
                        ...
                        var /** number */ distTreeIdx =
                                s.distContextMap[
                                    s.distContextMapSlice + distanceCode
                                ] & 0xff;
                        ...
                        if (distanceCode < 16) {
                            ...
                            s.distance =
                                s.rings[index] +
                                DISTANCE_SHORT_CODE_VALUE_OFFSET[distanceCode];
                            if (s.distance < 0) {
                                throw 'Negative distance';
                            }
                        } else {
                            var /** number */ extraBits =
                                    s.distExtraBits[distanceCode];
                            var /** number */ bits;
                            ...
                            s.distance =
                                s.distOffset[distanceCode] +
                                (bits << s.distancePostfixBits);
                        }
                    }
                    if (
                ...
                case 8:
                    var /** number */ src =
                            (s.pos - s.distance) & ringBufferMask;
    ...
    function transformDictionaryWord(
        ...
        var /** !Int8Array */ prefixSuffixStorage =
            ...
            while (len > 0) {
                ...
                if (c0 < 0x80) {
                    ...
                    if (len >= 4) {
                        ...
                        dst[shiftOffset + 1] =
                            (c1 & 0xc0) | ((scalar >> 12) & 0x3f);
                        dst[shiftOffset + 2] =
                            (c2 & 0xc0) | ((scalar >> 6) & 0x3f);
    ...
    function copyBytes(s, data, offset, length) {
        ...
        if (copyNibbles > 0) {
            ...
            data.set(
                s.byteBuffer.subarray(readOffset, readOffset + delta),
                offset,
            );
            offset += delta;
            length -= delta;
...
onmessage = function (event) {
    let { pointAttributes, scale, name, min, max, size, offset, numPoints } =
        event.data;
        ...
        try {
            ...
            buffer = {
                buffer: new ArrayBuffer(
                    numPoints * (pointAttributes.byteSize + 12),
                ),
            };
